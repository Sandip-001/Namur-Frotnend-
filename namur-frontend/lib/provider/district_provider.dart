...
class DistrictProvider with ChangeNotifier {
  ...
  List<String> setDistrict(
    ...
    // 🔥 Find original key (case-insensitive) for JSON lookup
    final originalKey = _fullJson.keys.firstWhere(
    ...
    _selectedDistrict = _districts.firstWhere(
      ...
      orElse: () => _capitalize(originalKey),
    );

    final districtData = _fullJson[originalKey] as Map<String, dynamic>;
  ...
  List<String> setTaluk(
    ...
    // 🔥 Find original keys for JSON lookup
  ...
  void setVillage(String value) {
    ...
    _panchayats = _villages.map((v) => _capitalize(v)).toList();

    // Reset Panchayat if not in list
    if (_selectedPanchayat != null && !_panchayats.any((p) => p.trim().toLowerCase() == _selectedPanchayat!.trim().toLowerCase())) {
      _selectedPanchayat = null;
    }
  ...
  Future<bool> saveBasicDetails({
    ...
    final url = Uri.parse(ApiConstants.saveBasicDetails);
    print('save basic url$url');
    ...
    if (response.statusCode == 200) {
      // 🔥 CRITICAL: Update local storage so splash screen knows we are done
  ...
  // Save Address API
  // ---------------------------------------------------------
  Future<bool> saveAddressDetails() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? uid = prefs.getString("firebase_uid");
    if (uid == null) return false;

    final url = Uri.parse(ApiConstants.saveAddressDetails);
