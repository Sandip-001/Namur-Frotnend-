...
class MachineryAdCard extends StatefulWidget {
  final MachineryAdModel ad;
  final VoidCallback? onTap;

  const MachineryAdCard({super.key, required this.ad, this.onTap});

  @override
  State<MachineryAdCard> createState() => _MachineryAdCardState();
}

class _MachineryAdCardState extends State<MachineryAdCard> {
  ...
  Widget build(BuildContext context) {
    final imageUrl = widget.ad.images.isNotEmpty
        ? widget.ad.images.first.url ?? ""
        : "";
    ...
    final taluk = widget.ad.userTaluk?.toTitleCase();
    final village = widget.ad.userVillage?.toTitleCase();
    ...
    final dateText = DateFormat(
      'dd-MM-yyyy',
    ).format(DateTime.parse(widget.ad.expiryDate));
  ...
  String _buildDistrictDisplay(List<String> districts) {
    ...
    if (districts.length == 1) return districts.first.toTitleCase();
    final extra = districts.length - 1;
    return "${districts.first.toTitleCase()} +$extra dist";
  }
